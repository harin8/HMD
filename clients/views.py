        ...
        if group_name and party_name:
            ...
            for i, client_id in enumerate(client_ids):
                ...
                if client_id and ( party_name or it_no or audit_no  or it_size or audit_size):
                    ...
                    party_detail = database.get_party_detail_from_id(r_id)
                    client_list = database.get_all_distinct_clients_from_party_name(party_detail['Party_name'])
                    return render(request, 'new_party_transfer.html', {'Error': True, 'Alert': "Error while updating the database",
...
def password_validate(request):
    ...
    if operation_name == "Party Creation":
        if pwd == "HMDDIWAN":
            return JsonResponse({'Alert': "Correct"})
    if operation_name == "Group Creation":
        if pwd == "HMDDIWAN":
            return JsonResponse({'Alert': "Correct"})
    if operation_name == "Party Closure":
        if pwd == "HMDDIWAN":
            return JsonResponse({'Alert': "Correct"})
    if operation_name == "Client Closure":
        if pwd == "HMDDIWAN":
            return JsonResponse({'Alert': "Correct"})
    if operation_name == "Group Closure":
        if pwd == "HMDDIWAN":
            return JsonResponse({'Alert': "Correct"})
    if operation_name == "Party Transfer":
        if pwd == "HMD1231":
            return JsonResponse({'Alert': "Correct"})
    if operation_name == "Client Edit":
        if pwd == "HMD1231":
            return JsonResponse({'Alert': "Correct"})
    if operation_name == "Contact Edit":
        if pwd == "HMD1231":
            return JsonResponse({'Alert': "Correct"})
